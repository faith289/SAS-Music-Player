...
class SASPlayer(QMainWindow):
    ...
    def show_color_settings(self):
        ...
        def toggle_auto_color():
            ...
            if not current and hasattr(self, 'album_original_image') and self.album_original_image:
        ...
        # Add the modern color palette (enabled/disabled based on auto-color setting)
        from color_settings import ModernColorPalette
        palette = ModernColorPalette(dialog)
        palette.color_selected.connect(lambda color: self.on_manual_color_selected(color))
        main_layout.addWidget(palette)
        
        # Function to update palette state based on auto-color setting
        def update_palette_state():
            is_auto_enabled = self.color_settings.get_auto_color_from_album()
            palette.setEnabled(not is_auto_enabled)
            opacity = "0.4" if is_auto_enabled else "1.0"
            palette.setStyleSheet(f"QWidget {{ opacity: {opacity}; }}")
        
        update_palette_state()
        
        # Add separator line
        separator2 = QLabel()
        separator2.setStyleSheet("background-color: #444; height: 1px; margin: 5px 0;")
