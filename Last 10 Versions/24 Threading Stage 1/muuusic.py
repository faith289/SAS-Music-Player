...
# Handle VLC plugin path for frozen builds
...
from PyQt6.QtCore import (
    Qt, QTimer, pyqtSignal, pyqtSlot, QRect, QEvent, QSize,
    QPropertyAnimation, QEasingCurve, pyqtProperty
)
from PyQt6.QtGui import (
    QPixmap, QLinearGradient, QImage, QFont, QFontMetrics, QColor, QPalette,
    QIcon, QPainterPath, QPainter, QBrush, QPen, QAction
)
from PyQt6.QtWidgets import (
    QApplication, QMainWindow, QWidget, QLabel, QPushButton, QStyledItemDelegate,
    QSpacerItem, QListWidget, QListWidgetItem, QSlider, QHBoxLayout, QSizePolicy,
    QVBoxLayout, QFileDialog, QMenu, QGraphicsBlurEffect, QGraphicsDropShadowEffect,
    QSystemTrayIcon, QGraphicsOpacityEffect, QDialog, QGridLayout
)
...
from widgets import (
    ScrollingLabel, GlowButton, ShadowLabel, ReorderablePlaylist,
    PulsingDelegate, AlbumArtWidget
)
from styles import (
    SIDEBAR_BG_COLOR, SIDEBAR_HOVER_COLOR, SPOTIFY_GREEN, SPOTIFY_GREEN_HOVER,
    WHITE, BLACK, ICON_PLAY, ICON_PAUSE, ICON_NEXT, ICON_PREV,
    ICON_BRIGHTNESS, ICON_APP, BUTTON_STYLE_TRANSPARENT, BUTTON_STYLE_SIDEBAR,
    BUTTON_STYLE_SIDEBAR_ACTIVE, VOLUME_BUTTON_STYLE,
...
from utils import (
    format_time, is_audio_file, calculate_brightness,
    get_safe_basename, create_color_variants
)
...
from workers.metadata_worker import MetadataManager
...
class SASPlayer(QMainWindow):
    ...
    def _final_close(self):
        ...
        try:
            ...
            print(f"Metadata cleanup error (non-fatal): {e}")
    ...
    def load_songs(self):
        ...
        if files:
            ...
            for f in files:
                ...
                self.metadata_manager.add_file(f)
    ...
    def on_metadata_ready(self, filepath: str, metadata: dict):
        try:
            if 0 <= self.current_index < len(self.playlist):
                ...
                if os.path.abspath(filepath) == os.path.abspath(current_path):
                    ...
                    title = (
                        metadata.get("title")
                        or os.path.basename(current_path)
                        or self.song_label.text()
                        or "Unknown Title"
                    )
                    # Update combined meta info
                    self.meta_label.setText(f"{artist} — {album} — {title}")
                    # Update the visible song title label
    ...
    def on_metadata_error(self, filepath: str, error: str):
        try:
            if 0 <= self.current_index < len(self.playlist):
                current_path = self.playlist[self.current_index]
                if os.path.abspath(filepath) == os.path.abspath(current_path):
                    self.meta_label.setText("Metadata not available")
        except Exception:
            pass
    ...
    def dropEvent(self, event):
        for url in event.mimeData().urls():
            ...
            if is_audio_file(file_path):
                ...
                self.metadata_manager.add_file(file_path)
