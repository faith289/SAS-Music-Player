...
from PyQt6.QtGui import QPixmap, QImage, QFont, QColor, QPalette, QIcon, QPainter, QBrush, QPen, QAction
from PyQt6.QtCore import Qt, QTimer, pyqtSignal, pyqtSlot, QRect, QEvent
import vlc
...
class FAiTHPlayer(QMainWindow):
    ...
    def toggle_play_pause(self):
        if self.player.is_playing():
            self.player.pause()
            self.play_btn.setText("▶")
            self.toggle_action.setText("▶️ Play")
        else:
            self.player.play()
            self.play_btn.setText("⏸")
