...
from PyQt6.QtWidgets import (
    ...
    QGraphicsOpacityEffect, QDialog, QVBoxLayout, QLabel, QSlider, QPushButton, QHBoxLayout
)
...
class FAiTHPlayer(QMainWindow):
    ...
    def _setup_background_layers(self):
        ...
        # Removed the original mini_btn from the top area
    ...
    def resizeEvent(self, event):
        ...
        self.bg_blur_label_2.setGeometry(0, 0, self.width(), self.height())
        self.dark_overlay.setGeometry(0, 0, self.width(), self.height())
        # Move brightness button to far top right, very close to border (same as sleep button)
        margin = 4
        self.brightness_btn.move(self.width() - self.brightness_btn.width() - margin, margin)
        #self.mini_btn.move(self.width() - 50, 44)  # No longer needed for bottom row
    ...
    def show_sleep_timer_menu(self):
        ...
        result = dlg.exec()
        self.set_blur(False)
        # Do NOT immediately hide/delete the blur overlay here; let the fade-out animation handle it.
        if hasattr(self, '_blur_overlay'):
            self._blur_overlay.hide()
        # (No overlay_widget hide/delete here!)
        # Double guarantee: immediately remove overlay if it still exists
        ...
        if hasattr(self, '_blur_overlay'):
            self._blur_overlay.hide()
            # Add a temporary click-blocking overlay for 350ms
            from PyQt6.QtWidgets import QWidget
            class DelayClickOverlay(QWidget):
                def __init__(self, parent):
                    ...
                    self.show()
                    self.raise_()
            self._delay_click_overlay = DelayClickOverlay(self)
            QTimer.singleShot(350, lambda: (self._delay_click_overlay.hide(), self._delay_click_overlay.deleteLater(), setattr(self, '_delay_click_overlay', None)))
        if timer_active and getattr(dlg, 'stop_clicked', False):
            ...
            if getattr(dlg, 'use_end_of_song', False):
                if self.controller and self.controller.duration_ms > 0:
                    ...
                    minutes = max(1, int(remaining_ms // 60000))
