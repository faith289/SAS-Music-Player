...
from PyQt6.QtCore import Qt, QTimer, pyqtSignal, pyqtSlot, QRect, QEvent, QSize
import vlc
...
class FAiTHPlayer(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.playlist_widget.setItemDelegate(PulsingDelegate(
        self.playlist_widget, lambda: self.current_index
        ))
        ...
        shadow.setXOffset(0)
        shadow.setYOffset(4)
        shadow.setColor(QColor(0, 0, 0, 160))  # soft black shadow
        ...
        # --- Group album art + song + metadata ---
        self.meta_layout = QVBoxLayout()
        self.meta_layout.setSpacing(8)  # control spacing between items
        self.meta_layout.setContentsMargins(0, 0, 0, 0)

        self.meta_layout.addWidget(self.album_art_label, alignment=Qt.AlignmentFlag.AlignCenter)
        self.meta_layout.addWidget(self.song_label, alignment=Qt.AlignmentFlag.AlignCenter)
        self.meta_layout.addWidget(self.meta_label, alignment=Qt.AlignmentFlag.AlignCenter)
        ...
        self.apply_shadow(self.elapsed_label, blur_radius=6, x_offset=1, y_offset=1)
        self.apply_shadow(self.duration_label, blur_radius=6, x_offset=1, y_offset=1)
        ...
        button_style = """
            ...
            QPushButton:hover {
                background-color: rgba(30, 120, 80, 0.6);
                border: 1px solid rgba(255, 255, 255, 0.3);
            }
            QPushButton:pressed {
                background-color: rgba(30, 120, 80, 0.8);
            }
        """
        ...
        self.play_btn.setIcon(self.play_icon)  # start with play icon
        ...
        for btn in (self.prev_btn, self.play_btn, self.next_btn):
            ...
            glow.setColor(QColor(0, 255, 150, 180))  # teal glow
            glow.setOffset(0, 2)
        ...
        # 1. Grouped album art, song name, artist
        self.main_layout.addLayout(self.meta_layout)
        self.main_layout.addSpacing(15)

        # Group all playback-related widgets below metadata
        self.bottom_controls_layout = QVBoxLayout()
        self.bottom_controls_layout.setSpacing(12)  # even vertical spacing

        self.bottom_controls_layout.addLayout(self.seek_row)
        self.bottom_controls_layout.addLayout(self.controls)
        self.bottom_controls_layout.addWidget(self.volume_slider)

        # Add the grouped controls to main layout
    ...
    def load_song(self, index):
        ...
        playing_icon = QIcon("▶️")  # use emoji or load from file
