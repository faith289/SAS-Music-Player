...
class MinimalMusicPlayer:
            ...
            try:
                ...
                for tag in audio.tags.values():
                    if isinstance(tag, APIC):
                        img = Image.open(io.BytesIO(tag.data)).resize((200, 200), Image.Resampling.LANCZOS)
                        img = self.rounded_album_art(img, 30)
                        self.album_art_cache[file_path] = img
                        self.root.after(0, lambda: self.fade_album_art(img))
                        return
            except:
                pass
            self.root.after(0, self.set_default_album_art)

        threading.Thread(target=load_art, daemon=True).start()

    def rounded_album_art(self, img, radius):
    ...
    def set_default_album_art(self):
        try:
            ...
            image = self.rounded_album_art(image, 30)
    ...
    def fade_album_art(self, new_image_pil):
        ...
        def blend(alpha):
            if alpha <= 1.0:
                blended = Image.blend(self.album_original_image, new_image_pil, alpha) if self.album_original_image else new_image_pil
